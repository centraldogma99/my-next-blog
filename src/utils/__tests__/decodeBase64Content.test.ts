...
describe('decodeBase64Content', () => {
  ...
  it('한글 텍스트를 올바르게 디코딩한다', () => {
    ...
    expect(decodeBase64Content(base64)).toBe(koreanText)
  })
...
- 목록 2`
    ...
    expect(decodeBase64Content(base64)).toBe(markdown)
  })

  it('빈 문자열을 디코딩한다', () => {
    const base64 = Buffer.from('', 'utf-8').toString('base64')
    
    expect(decodeBase64Content(base64)).toBe('')
  })
  ...
  it('Buffer 클래스가 존재하고 정상 작동한다', () => {
    ...
    expect(decodeBase64Content(base64)).toBe(text)
